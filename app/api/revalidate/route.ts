...
export async function POST(req: Request) {
  ...
  if (!signature) {
    return NextResponse.json(
      { message: 'No signature found in headers' },
      { status: 401 }
    );
  }
  ...
  try {
    ...
    if (remoteBranch === `refs/heads/${branch}`) {
      ...
      for (const slug of changedPostSlugs) {
        ...
        await fetch(`https://${subdomain}.datadiary.dev/blog/${slug}`, {
          method: 'GET'
        });
      }
      ...
      return NextResponse.json(
        { message: `Different branch in payload: ${remoteBranch}` },
        { status: 400 }
      );
    }
